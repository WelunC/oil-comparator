...
if (savedTheme === 'light') {
  ...
  document.body.classList.add('dark-mode');
    ...
    let allStats = Array.from(
      new Set(data.flatMap(d => d.stats.map(s => s.split(':')[0].trim())))
    );
    ...
    function render() {
      ...
      for (const oil of filtered) {
        ...
        div.innerHTML = `
          <a href="${oil.wiki_link}" target="_blank" rel="noopener noreferrer">
            <strong>${oil.name}</strong>
          </a>
          <p>Price: ${oil.price}</p>
          <ul>
            ${oil.stats.map(s => {
              const match = [stat1, stat2]
                .filter(Boolean)
                .some(f => normalize(s).includes(normalize(f)));
    ...
    filter1.onchange = render;
    filter2.onchange = render;
